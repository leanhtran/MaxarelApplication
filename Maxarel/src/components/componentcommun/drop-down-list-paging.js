...
import {
    StyleSheet, Text, View, FlatList, TouchableHighlight, Modal, StatusBar, StatusBarIOS,
    TouchableWithoutFeedback, TextInput, Platform, Dimensions, PanResponder, Animated, KeyboardAvoidingView
} from 'react-native';
import Icon from 'react-native-vector-icons/Ionicons';
const { height, width } = Dimensions.get("window");

class DropdownPaging extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    _renderDropdownElements = () => {
        ...
        return (
            <FlatList style={{ height: this.state.heightList }}
                onEndReached={this._loadMoreData}
                data={data}
                renderItem={({ item }) => (
                    <TouchableHighlight
                        key={item.key}
                        onPress={() => this._onPressButton(item)} underlayColor="white">
                        <View >
    ...
    _loadMoreData = () => {
        this.setState({
            data: [...this.state.data, { key: "id11", label: "test11" }, { key: "id12", label: "test13" }]
        })
    }
    ...
    _renderRightButton = () => {
        return (
            <TouchableWithoutFeedback hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
                onPress={() => {
                    this.setState({
                        currentPage: Math.min(this.state.currentPage + 1,
                            this.state.maxPage)
                    });
                }}>
